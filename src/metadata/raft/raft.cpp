...
void Raft::Elect(const ::raft::RequestVoteRequest &request)
{
    ...
    // 使用 std::execution::par 进行并行化操作
    std::for_each(std::execution::par, _peers.begin(), _peers.end(), [&request, &votes, this](auto peer) {
        if (!peer) return ; // me
...
void Raft::AppendEntries()
{
    ...
    // std::execution::par 并行化
    std::for_each(std::execution::par, _peers.begin(), _peers.end(), [&request, this](auto peer) {
        if (!peer) return ; // me

        // 当前 peers 的 index
        size_t index = std::distance(_peers.begin(), peer);
    ...
    {
        {
            ...
            for (uint64_t idx = _lastApplied; idx <= _commitIndex; ++ idx)
            {
                ApplyMsg msg;
                    ...
                    ::google::protobuf::Closure* done)
{
    ...
    {
        ::raft::LogEntry log = request->entries(0);
