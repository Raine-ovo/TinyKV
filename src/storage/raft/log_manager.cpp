...
Entry LogManager::get(uint64_t index)
{
    if (index < lastIncludedIndex) {
        LOG_ERROR("LogManager::get index too small! index: {}, lastIncludedIndex: {}", 
    ...
    {
        LOG_ERROR("LogManager::get outbound!, index: {}, last_index: {}", index, entries.size()-1);
        exit(EXIT_FAILURE);
    }
...
void LogManager::drop(uint64_t index, bool set)
{
    uint64_t log_index = index - lastIncludedIndex;
    ...
    if (set)
    {
        set_lastIncludedIndex(index);
        set_lastIncludedTerm(get(index).term);
    }
}
